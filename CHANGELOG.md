...
+ [生产换绑/验收止损] 2026-07-22：account hash `3b18db641494` 保持原 fp/session，请求 shape 不变，生产 sticky Webshare 从 `82.29.223.111` 换绑到 `45.39.75.27`。串行 5 实际执行 4 次：前 3 次均触发 `image_gen` 并下载 PNG，第 4 次 `no_image_gen_within_45s`，按门禁立即停止，未执行第 5 次、未整单重试、未换号、未启动并发 4。原始 harness 业务 CF 分类为 0，但首页软失败 403 后经复盘确认为 `4/4`；结束后 `healthy=true`、调度 `10/10`、`inflight=0`，新绑定保留，回滚快照已留存。证据 `docs/captures/spa/M-panda-new-ip-serial5-20260722.json`。
+ [生产 A/B/裁决] 2026-07-22：同账号、同 fp、同 request shape 仅切换 Webshare IP。新 IP `45.39.75.27` 连续 2 次无首页/`tasks` CF403 且出图下载成功；旧 IP `82.29.223.111` 单次未复现 CF403，但下载 `503 ServerBusy`。严格旧 IP 主因标准未满足，当前按间歇性 edge/endpoint/timing 处理；生产绑定未改、无整单重试。证据 `docs/captures/spa/L-panda-webshare-ip-ab-20260722.json`。
+ [生产验收/止损] 2026-07-22：固定单账号/单 Webshare 串行 5 按 `300s/request`、`1800s/run`、`45s image_gen deadline`、`0.5 CPU / 512 MiB / concurrency=1` 执行；连续两轮出现 CF 信号后停止在 2/5。第 1 轮 `no_image_gen`，第 2 轮成功下载 PNG `2,475,088` bytes；未换号、未整单重试、未启动并发 4。结束后 `healthy=true`、调度 `10/10`、`inflight=0`。证据 `docs/captures/spa/K-panda-production-pure-http-serial5-20260722.json`。
+ [部署/验证] 2026-07-22：严格纯 HTTP 生图 4 个运行时文件已通过 Git/artifacts 正式部署 Panda，artifact `650e899084c319ede7436c7b9497b4af9b991eba`，备份 `/root/gptimage/backups/pure-http-prod-20260722-144517`；发布后 `healthy=true`、`image_schedulable=10`、`dispatchable=10`、`inflight=0`、`startup_errors=0`。
+ [生产 canary/风控] 2026-07-22：单账号、单 Webshare、单请求纯 HTTP canary 成功；conversation `6a606849-e1b8-83ec-96e4-e7cfbbbf305b`，`has_image_gen=true`，下载 PNG `1254×1254`、`2,568,782` bytes，SHA256 `1f886e15532bfc9973897a9d285f311ecadeedaa0346dc0101df25629e5fa5bd`，总耗时 `42,246ms`。`/tasks` 1 次 CF403 后 conversation poll 恢复，未整单重试、未换号；按门禁停止，未做串行 5 / 并发 4。证据 `docs/captures/spa/J-panda-production-pure-http-canary-20260722.json`。
+ [验证/风控] 2026-07-22：Panda 隔离 staging 使用另一账号与账号级 Webshare 单 canary，纯 HTTP SSE 已触发 `image_gen` 并收到 sediment ID；poll/download 阶段同一出口连续两次 CF403，未下载 PNG。脱敏原子证据 `docs/captures/spa/I-panda-webshare-pure-http-canary-20260722.json`；未换号、未重试。
+ [修复/测试] 2026-07-22：poll 连续 CF 达阈值后直接抛出并阻止 resolver 继续请求附件，避免放大同一出口挑战；新增回归，定向 `34 passed`、扩展受影响回归 `92 passed`。该条记录为正式发布前的本地验证阶段，生产发布结果见上。
+ [协议/修复] 2026-07-22：Turnstile VM 已被真实 finalize 接受并取得一次 curl_cffi 纯 HTTP 真出图（conversation `6a602f69…`，PNG 841,139 bytes）；默认启用无 conduit auto-tool envelope。正式链路继续剥离空 user metadata/create_time/trace，并将 OAI-Language 按时区推导。
+ [风控/稳定性] 2026-07-22：生图 requirements 遇 CF403 改为单次 fail-fast，避免同号三连；TLS/socket 重建保留原 proxy/verify/impersonate/timeout。canary 不再把仅有 conversation_id 当成功；相关回归 `85 passed`。该条记录为正式发布前阶段，现网结果见上。
+ [文档] 2026-07-22：严格纯 HTTP 生图全量清单 `docs/20-pure-http-image-sentinel-todo.md`；`04` 新增 **PROTO-PURE-HTTP**（P0）。初建清单时 P2 仍阻塞，已被同日 VM 突破与真出图记录更新。
+ [验证] 2026-07-21：生图 bench 加**「N 秒无 image_gen 立刻失败」**（`--image-gen-deadline`）+ `--protocol {picture_v2,spa_tool}`（`spa_image_load_test`/`_tmp_spa_image_bench3` 同步透传，summary 增 `no_image_gen`）。Panda 冷却号跨 **3 个出口 IP 段 × 多设备**实测：`picture_v2`（conduit+`["picture_v2"]`+`X-Conduit-Token`）与空 hints 文本 shape **均不进 `image_gen`**；生产 `/v1/images/generations` 同步超时，`image_poll_timeout ... last_task_error: null`（**非 CF**）。定性：非单 IP/非单设备风控，主缺口 Sentinel/Turnstile → `04` **PROTO-PURE-HTTP** / `20`。报告 `docs/captures/spa/G-image-gen-not-triggered-20260721.md`；刷新 `12`。
+ [功能] 2026-07-21：号池「代理/出口」把 provider（如 webshare）灰字移到 IP 右侧；IP 漂移灯下增加 **CF 状态灯**（被动 `cf_daily`：绿无403 / 黄偶发 / 红偏多 / 灰无样本）；**不做**主动 CF 探测。
+ [修复] 2026-07-21：生图 poll 连续 CF 快失败（`image_poll_cf_abort_streak` 默认 2）+ tasks 遇 CF 跳过；纳入 `image_stream_cf_failover`；压测 summary 标 `cf_abort`；刷新 `docs/17`。
+ [文档] 2026-07-21：同步 SPA 挖矿完成态；新增待办 **`04` PROTO-REFACTOR（按逆向结果改造生产）**——优先上传链 `process_upload_stream`+`sediment://`；刷新 `02`/`06`/`12`/`18`/`19`/`README`。
+ [协议/验证] 2026-07-21：执行 `docs/19` Next——D 上传链（files→oaiusercontent PUT→process_upload_stream）+sediment 对照+图生图形状；E HTTP `system_hints:["search"]` 开/关 200（SPA UI「Look something up」）；A cookie 剥离（Clash+Bearer 可空 Cookie）；差 IP Webshare 本机暖机 NET_RESET（对照 Clash/bench3）。专页 `A-cookie-strip`/`A-badip-warm`/`D-upload-sediment`/`E-search`。
+ [功能] 2026-07-21：Outlook 号池「恢复」支持密码重登 + YuMail/mailManage OTP（默认环回 `127.0.0.1:8782/api/v1`，避公网 CF）；无邮箱凭据文件也可启动；可选 `OUTLOOK_RECOVERY_BACKEND=camoufox`。
+ [功能] 2026-07-21：补齐 `yumail` mail provider + `services/yumail_otp.py`；Key 走 `YUMAIL_API_KEY` / `data/runlogs/yumail_api_key.secret.txt`。
+ [功能] 2026-07-21：SPA 生图 feature flag `image_spa_tool_path`（默认 false）+ 串行/并发压测 `scripts/spa_image_load_test.py`；生产默认仍 `picture_v2`+conduit。
+ [修复] 2026-07-21：SPA start 头统一走 `build_image_start_headers`（含 `X-Oai-Turn-Trace-Id`）；YuMail 不可达且有邮箱凭据时回退 Graph；`check_prerequisites` 与 `_validate_target` 探针对齐。
+ [协议/验证] 2026-07-21：执行 `docs/19` Now——Create Image UI HAR 确认仍发 `system_hints:["picture_v2"]`+conduit（生产保持该形）；续聊 N=3+10 字段消融 OK；Camoufox→curl_cffi 暖机 SSE OK；错误码字典 v1；panda SOCKS SSE 工程收口。专页 `docs/captures/spa/{A,B,C,F}-*-20260721.md`；刷新 `12`/`19`。
+ [协议] 2026-07-21：SPA HAR 补证（Clash+固定号 `qaflow0ytb7…@proton.me`）→ 文本改 `/f/conversation`+prepare、Sentinel Prepare-Token、OAI Client Version/Build 跟现网、chat body 对齐 SPA 字段；落盘 `docs/captures/spa/`（HAR gitignore + `field-diff-20260721.md`）；HTTP 文本复现 OK（curl_cffi）；HTTP 生图按 SPA 同形（`system_hints=[]`、无 `X-Conduit-Token`）经 Camoufox APIRequest 复现 OK（`has_image_gen`；curl_cffi 同出口 TLS 挂时改浏览器栈）。反代 `/v1/images` 仍保留 `picture_v2`+conduit。不改 `17` CF 裁决。
+ [文档] 2026-07-21：协议全量逆向目录 `docs/19-protocol-full-reverse-catalog.md`（A–F 分层、机房 IP 前提、看板/验收；Webshare 不作「高质量代理」假设）；索引 `docs/README` / `12` / `18`。
+ [验证] 2026-07-21：SPA HTTP 生图+下载三轮对照（中等复杂度 prompt）— 本地 Clash OK（78.4s / 图 2.51MB / 流量 3.13MB）；panda 直连 CF403 FAIL；panda 账号 Webshare OK（56.9s / 图 2.32MB / 流量 2.44MB）。报告 `docs/captures/spa/bench3-20260721.md`。
+ [验证] 2026-07-21：panda IP（`43.156.233.219`）经 `ssh -D` 链式到本机 + Camoufox：可过 CF `home`/`chat-requirements/prepare`（容器内 curl_cffi 同 IP 则 403）；SSE 全链路未收口。报告 `docs/captures/spa/panda-socks-camoufox-20260721.md`。
+ [文档] 2026-07-21：固化 CF 403 裁决 `docs/17-cf403-and-egress.md`（成因/历史/不可协议绕过/已缓解）；SPA 逆向反代任务书 `docs/18-openai-web-reverse-proxy-brief.md`；刷新 `02`/`06`/`12`/`README`。
+ [修复] 2026-07-21：生图任务回右侧；已完成按对话深浅灰交替；三段均可删（生成中/排队=取消源任务，已完成=仅藏列表）。IMAGE CF 403 快速换号（`image_stream_cf_failover`）+ bootstrap 硬失败；同 binding 同时生图默认≤1；错峰 1.5s；硬超时收紧（pre45+poll120）。artifacts `72ca9d5` → Panda，备份 `git-artifacts-deploy-20260721-104927`；health `healthy` / `image_schedulable=10` / `dispatchable=9` / `binding_inflight=1`；artifacts wipe。
+ [修复] 2026-07-21：生图 CF 边缘 403 不再误报为 connection failed；`submit_start_min_interval_ms` 默认/现网 5s→0.8s 解除假串行；任务列表移最左并分「正在生图/排队/已完成」；左右用时统一按提交墙钟。artifacts `a4fbf71` → Panda，备份 `git-artifacts-deploy-20260721-102504`；health `image_account_concurrency_limit=4` / `image_global_concurrency_limit=10` / `image_schedulable=10` / `dispatchable=8`；artifacts wipe。
+ [文档/运维] 2026-07-21：固化新号固定链路（取号→未占用 Webshare→探活+邮箱预检→Camoufox→blob 入库观察）；入口 `scripts/outlook_camoufox_stable_register.py`；刷新 `docs/16-camoufox-stable-pipeline.md`、`02`/`06`。样例：`CharlieTim7490@outlook.com` + `92.113.246.215`。
+ [修复] 2026-07-21：生图多槽提交改 `Promise.allSettled`，单槽失败不再整批标红；已成功入队的图继续轮询。artifacts `4ad3d4d` → Panda，备份 `git-artifacts-deploy-20260721-100310`；health ok；artifacts wipe。+ [功能] 2026-07-21：同 prompt 生图去重允许多图并行（默认 4）；生图任务可取消；号池展示懒刷新/拟人对话倒计时图标；生图页右侧任务列表；会话 IndexedDB 去 base64 减卡顿；默认单账号并发 4。artifacts `bdf2290` → Panda，备份 `git-artifacts-deploy-20260721-095846`；health `image_account_concurrency_limit=4` / `image_schedulable=10`；artifacts wipe。
+ [功能] 2026-07-21：同 Webshare `proxy_binding` 承载上限默认 **5**（`proxy_binding_max_accounts`）；≤5 可同 IP 注册/进生图调度，超限才隔离/挡候选。artifacts `7dc7562` → Panda，备份 `git-artifacts-deploy-20260721-092434`；health `image_schedulable=10`（此前因 dup 仅 5）；artifacts wipe。
+ [功能] 2026-07-21：号池流水拆「api生图/对话生图/拟人对话/真实对话」；真实对话按 CALL「文本生成」计入；可调度=人工进调度；删轻量保活；自动恢复顶条；对话侧栏加宽/置顶/无滚动条/切会话防抖；代码块自动着色。artifacts `d09a05f` → Panda，备份 `git-artifacts-deploy-20260721-090236`；health `schedulable=10`；artifacts wipe。
+ [修复] 2026-07-20：`qaflowud630wbo2a@proton.me` 换绑 `92.113.246.12:5597`；旧端点 `92.113.246.215:5800` 写入 `gpt_unavailable_proxies.json`；对话页生图意图路由/粘号不可调度降级；输入框 1–6 行自适应；去掉过时 Panda 接收/ready 卡片；号池首屏与 usage 解耦加速。artifacts `9b58b61` → Panda，备份 `git-artifacts-deploy-20260720-223909`；health `healthy True` / `schedulable=5`；artifacts wipe。
+ [运维] 2026-07-20：对话 UX（代码高亮/复制/编辑重来/轮次导航/放大布局）+ conversation CF403 重试换号；artifacts `1bc247e` → Panda，备份 `git-artifacts-deploy-20260720-214520`；health ok；artifacts wipe。
+ [运维] 2026-07-20：对话/运维/号池大改 artifacts `5b6b6bf` → Panda overlay；备份 `git-artifacts-deploy-20260720-211348`；health `healthy True` / `schedulable=6` / `inflight=0`；artifacts 已 wipe。
+ [功能] 2026-07-20：对话/运维/号池大改——常驻预热池轮换（并发≥3 demote）、chat 走 `_ensure_bootstrap`、chat-requirements CF 403 有限重试、DeepSeek JSON Extra data 加固；对话多会话并发≤4、模型下拉、状态只显示耗时、粘账号/归档分组/导出；搜索记 egress；运维改名+拟人摘要/流量 Top 迁入；KPI 可派发同源；半小时图横坐标修复；号池序号+多键排序+记录柱懒加载。
+ [功能] 2026-07-20：联网 `/v1/search` 真流式（`stream:true` → chat.completion.chunk + sources）；对话页联网改 SSE 边出字；artifacts `fc26e82`/`a5a51ba` → Panda；实测文本 TTFT **6.99s** / 整包 **7.69s**（原先整包约 9–10s 才见字）。
+ [优化] 2026-07-20：联网搜索 SSE 边收边推（不等 [DONE]）+ bootstrap PoW 缓存 + prepare∥requirements；artifacts `d91683c` → Panda；实测 COLD TTFT **4.76s**（此前约 7s 才见字）。
+ [功能] 2026-07-20：联网 `/v1/search` 真流式（`stream:true` → chat.completion.chunk + sources）；对话页联网改 SSE 边出字；artifacts `fc26e82`/`a5a51ba` → Panda；实测文本 TTFT **6.99s** / 整包 **7.69s**。
+ [修复] 2026-07-20：生图支持 `X-Preferred-Account-Email`（`get_available_access_token(preferred_email=)`）；Panda 热修后基线 **文本 3/4、生图 3/4、self=0**（`rust-baseline-retry-20260720-173225`）；刷新 `docs/13`。
+ [优化] 2026-07-20：联网多模态降延迟——识图 SSE 直出、纯看图跳过二次搜索、bootstrap 并行、轮询 3s→1s；artifacts `46bf0d9` → Panda（备份 `git-artifacts-deploy-20260720-173203`）；实测 TEXT 9.5s / 看图 16.4s（fast）/ 看图+搜 28s。
+ [功能] 2026-07-20：联网 `/v1/search` 支持多模态——文本附件拼进 prompt；图片先识图再走文本搜索（规避上游 image+force_search 卡在 `search()` 桩）；前端联网可传附件。artifacts `acf4a51` → Panda，备份 `git-artifacts-deploy-20260720-171557`；实测 TEXT_ATTACH/IMAGE_MM 均 OK（浅灰图 33.9s）。
+ [文档] 2026-07-20：RUST-001 独立新项目 `gptimage-gateway-rs`（施工/契约/矩阵/操作指导）；本仓 `plan.md` 改指针、`14` 改索引；刷新 `13` 基线（空闲+文本/生图采样，runlogs `rust-baseline-*`）；修正 `02` 过时 `post_ready=15s`；明确砍注册机与 FlareSolverr，全量含 RCA。
+ [运维] 2026-07-20：对话十问 UX artifacts `8224c9a` → Panda；备份 `git-artifacts-deploy-20260720-162812`；health `healthy True` / `schedulable=6`；公网 marker「总耗时/准备中/上传图片或文本/对话鉴权」OK。
+ [修复] 2026-07-20：对话剥离泄漏 `search(...)`；灰底代码块；链接绝对化；固定高度仅消息区滚动；完成后消息下显示总耗时；附件上传（图/文本）；403/chat-requirements 与 deepseek_error 人性化文案。
+ [运维] 2026-07-20：对话续聊/真流式/链接/养号同步/风险 findings artifacts `8b8510d` → Panda；备份 `git-artifacts-deploy-20260720-160508`；health `healthy True` / `schedulable=6`；公网 marker「换号续聊/SSE 流式出文/真实对话完成/号池判定」OK。
+ [修复] 号池「对话」改同步 `process-one`（`accounts_ui` 在养号 OFF 时仍可强制执行）；SSE 加 `X-Accel-Buffering: no`；对话链接可点、用时计时、富文本加强；运维养号/风险徽章可读说明。
+ [运维] 2026-07-20：对话合并/流式/号池柱状+IP灯/运维X轴 artifacts `830560a` → Panda overlay；备份 `git-artifacts-deploy-20260720-152127`；health `healthy True` / `schedulable=6` / `inflight=0`；UI marker「联网/流式出文/未绑定账号/来源（」OK，熔断列表头 0。
+ [功能] 2026-07-20：运维半小时图 X 轴修复（双重 formatTick）；号池记录改双色柱状+快悬停、删熔断列、代理仅 provider、IP 漂移 7 灯、行级手动养号；对话页合并搜索/流式 SSE/浅色气泡/来源折叠/Skill 灯/指定账号；`egress_daily` 采样；旧身份 plan 归档并 Track L 结案。
+ [功能] 2026-07-20：号池软限流%可手动指定（5–99，标「手」；清空恢复自动）；成熟度/群组列改为「记录」6 日热力条+悬停；artifacts `429711a` → Panda，备份 `git-artifacts-deploy-20260720-121436`；健康 `schedulable=5` / `inflight=0`。
+ [运维] 2026-07-20：Panda artifacts `dd6c9e1` 部署（双图流水/风控自动化趋势/对话页/隐藏注册机与清障 UI）；备份 `git-artifacts-deploy-20260720-120002`；health ok；实测 `/v1/models` `/v1/search` `/v1/chat/completions` 均 200；`CHATGPT2API_DISABLE_REGISTER=1`。
+ [运维] 2026-07-20：Panda IP canary 两 Proton 已挂载**同一** Webshare `82.21.231.148:7462`（`cohort_id=proton_panda_ip_shared_webshare_20260720`）；T0 刷额度均 quota=25 / `verified_ready`，用于多日共享节点活性观察。
+ [运维/文档] 2026-07-20：Proton×2 经 Panda 直连 IP（`43.156.233.219`）Camoufox 注册成功并上传；`/backend-api` 直连刷额度 CF 403；两号 `identity_isolated`；`proxy_runtime` 恢复 `single_proxy`。固化产出文档 `docs/16-camoufox-stable-pipeline.md`；脚本 `scripts/proton_camoufox_panda_ip_register.py`。
+ [功能] 2026-07-20：账号流水拆双图；自动化趋势改为「检测/风控」风险因子；调试→对话（会话持久化/富文本/搜索常驻/Skill 探测）；注册机入口下线并停止自动启动；清障文案改为可选。
+ [运维] 2026-07-20：charts/汉化/自动化趋势 artifacts 部署 Panda（`f9da8c7`）→ overlay → recreate；备份 `git-artifacts-deploy-20260720-101207`；health `healthy True` / `schedulable=6`；UI marker「每日生图/自动化趋势/风险日历/软限流%」OK。
+ [功能] 2026-07-20：账号流水图增加「每日生图」「拟人对话」折线 + 日期范围/悬停；RCA 平滑多序列与自动化趋势百分比图；风险日历悬停与色阶；Soft/Maturity/Cohort 等表头汉化。
+ [优化] 2026-07-20：运维页全量汉化（队列深度/耗时均值/突发、排除原因、接收态、养号状态）；LLM 曲线改为业务失败+分序列归一化；巡检等级改确定性号池健康优先（DeepSeek JSON 解析失败不再抬 medium）。
+ [优化] 2026-07-20：风控时间线汉化简化（检查项/模型耗时/缺口）；懒刷新按账号哈希错峰（`scheduler.lazy_refresh_jitter_hours` 默认 6h），避免同窗 `restore_at` 齐刷。
+ [功能] 2026-07-19：拟人/防封/管道全量可视化 —— `risk_metrics` 半小时落盘、`/api/ops/humanlike-dashboard|risk-calendar|risk-checks`、半小时 DeepSeek→GPT 巡检（`risk_audit` 默认 OFF）、`/ops`「风控拟人」Tab、账号页 soft/maturity/cohort/流量/FP；Canvas `humanlike-risk-viz`；docs/11·04。
+ [运维] 2026-07-19 P0 存储：`image_tasks.db` 备份后 VACUUM（1575MiB/0 行 → **~0.02MiB**）；`logs.jsonl` 切割 gzip（163MiB → 热文件 0，归档 `logs.jsonl.20260719-211833.gz` 20MiB）；备份 `store-p0-20260719-211735`；health `schedulable=4`。
+ [文档] 2026-07-19：`14-rust-rewrite-plan.md`（RUST-001 分阶段规划）；`15-store-archive-and-log-rotation.md`（image_tasks VACUUM/停 b64、logs 轮转）；现场确认 `image_tasks.db`≈1575MiB 且 rows=0。
+ [功能] 2026-07-19：TEXT-NURTURE 队列（默认 OFF）+ L2 `/api/ops` 只读 facade 与 `/ops` 页；日志 `llm_ops` source/outcome 筛选；`proto_align_har_canary.py`；性能预估文档 `docs/13-performance-and-rewrite-estimate.md`。artifacts `4557743`，备份 `git-artifacts-deploy-20260719-210112`；health ok，UI marker「运维 RCA / LLM 操作日志」OK。
+ [功能] 2026-07-19：PROTO-ALIGN — 鉴权 tz/`OAI-Language` 跟 egress；养号开关 `text_chat_persist_history`/`chat_reuse_conversation` + 独立 text cid 续聊；contextual 抖动。SCHED-001 — `GET /api/accounts/schedulable-breakdown`，空配额 429 附带 breakdown。LLM-OPS — `llm_ops` 日志接入 L0 文本与 ai_review。artifacts 部署 `cb8f60a`，备份 `git-artifacts-deploy-20260719-203825`；health `schedulable=4`，breakdown 401 鉴权正常。
+ [文档] 2026-07-19：新增 `docs/11-llm-ops-and-text-nurture.md`、`docs/12-protocol-gap-vs-web.md`；README 按意图检索；`02`/`06` 摘要对齐 schedulable=4；风控裁决 **not yet proven**；`04` 登记 PROTO-ALIGN / SCHED-001 / LLM-OPS / TEXT-NURTURE。
+ [清理] 2026-07-19：`scripts/_tmp_*`×77 → `scripts/archive/tmp-20260719/`；删除 `chrome_*` 与旧 `web_dist.backup-*`（保留 `20260719-140316`）；清理根目录一次性探针文件。
+ [运维] 2026-07-19：调度开关 artifacts 部署 Panda（`cb7f39a` → overlay `api/accounts.py` + `account_service.py` + `web_dist`）；备份 `git-artifacts-deploy-20260719-140518`；health `schedulable=4`；UI marker「调度中/全部进调度」OK。
+ [功能] 账号页人工「进/出调度」：`POST /api/accounts/scheduling`（+ bulk）；进=`verified_ready`（`clear_isolation`），出=`identity_isolated`；与 status 禁用正交。
+ [运维] 2026-07-19：两 Proton 观察号已升 `verified_ready` 并刷额度，Panda `schedulable=4`。
+ [运维] 2026-07-19：从 `proton/registered_accounts.txt` 取 2 个 Proton 信箱，Camoufox+Panda Webshare 链注册 OpenAI，以 `identity_isolated` 入库观察（`qaflowud630wbo2a` / `qaflowakjewai6ps`）；脚本 `scripts/_tmp_proton_camoufox_openai_observe.py`。
+ [运维/修复] 2026-07-19 Panda：humanlike 部署曾把协议层回退成 `post_ready=15` + estuary **无鉴权 resource 下载**（`File stream access denied`）。已从备份 `git-artifacts-deploy-20260718-110441` 恢复 `openai_backend_api.py`（备份 `estuary-auth-restore-20260719-120031`）；iv/phillip 对话+生图均 `ok`；两号 `verified_ready` / `schedulable=2`。下次 artifacts 部署须带上该协议修复，避免再回归。
