...
def _try_enter_image_sync_admission(identity: dict[str, object] | None = None) -> tuple[bool, int]:
    ...
    with _IMAGE_SYNC_ADMISSION_LOCK:
        ...
        try:
            # extra_waiters is the seat index after admit; cap queue depth at seat limit.
            eta = int(
                image_task_service.estimate_sync_eta_secs(
                    identity,
                    extra_waiters=min(current_waiters + 1, limit),
                )
            )
        except Exception:
            eta = 0
...
async def _run_image_sync_call(call: LoggedCall, runner, **kwargs):
    try:
        ...
        # ImageTaskService._emit_pending_call_log already writes the rich
        # 「文生图/图生图调用完成」row (phase_timings / tokens / task_id).
        # Skip LoggedCall success log to keep one call log per task.
        return result
