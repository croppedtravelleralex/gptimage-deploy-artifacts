...
def create_router(app_version: str) -> APIRouter:
    ...
    @router.api_route("/images/{image_path:path}", methods=["GET", "HEAD"], include_in_schema=False)
    async def get_image(image_path: str):
        return get_image_response(image_path)

    @router.api_route("/image-thumbnails/{image_path:path}", methods=["GET", "HEAD"], include_in_schema=False)
...
<div class="card"><div class="label">真无限额</div><div class="value">{stats['unlimited_quota_count']}</div></div>
<div class="card"><div class="label">未知额度</div><div class="value">{stats.get('unknown_quota_count', 0)}</div></div>
