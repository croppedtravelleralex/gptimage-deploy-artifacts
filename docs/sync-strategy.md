...
> 状态说明：本文保留 Panda 同步设计。当前生产安全策略是同步默认关闭、`queue_on_failure=false`、失败账号留本地主池；本地计划任务 `gptimage-panda-account-sync` 已禁用。下文中 HMAC/nonce/大批量水位同步属于后续增强，不代表当前已开启。
...
| ---: | ---: | ---: | --- |
| >=1500 | 30~60min | <=20 | 慢同步，保护 Panda |
| 500~1499 | 15~30min | <=20 | 正常补货 |
| 200~499 | 5~15min | <=20 | 加速补货，仍受远端 batch cap 限制 |
| 100~199 | 3~5min | <=20 | 紧急补货，仍需人工确认 |
...
1. Nginx 限频。
2. Bearer admin key。
3. HMAC 签名（后续增强项，当前非必需前置）。
4. nonce 防重放（后续增强项）。
5. idempotency key 防重复写（后续增强项）。
6. 应用层按水位限流。
