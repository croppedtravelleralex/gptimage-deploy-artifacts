...
- [ ] 公网页面截图可见两列，宽屏展示正常，窄屏横向滚动可达。
- [ ] 公网 `/api/accounts` 对目标字段的 null/number/string 语义与前端一致。
- [ ] 18 条账号完成代理/出口/fp 证据分级，修复报告总数闭合为 18。
- [ ] 活跃账号级代理节点一对一校验通过，重复节点有明确归因和处置状态。
- [ ] 可恢复账号的 fp 经进程重启仍保持同一 hash；全部上游入口受统一门禁覆盖。
- [ ] 单 canary 通过 `1h / 6h / 24h / 72h` 观察，出口漂移、fp 漂移和新增终态均为 0。
