...
def _mtime_iso(path: Path) -> str:
    return datetime.fromtimestamp(path.stat().st_mtime, tz=BEIJING_TZ).strftime("%Y-%m-%d %H:%M:%S")


def _mtime_day(path: Path) -> str:
    return datetime.fromtimestamp(path.stat().st_mtime, tz=BEIJING_TZ).strftime("%Y-%m-%d")
        ...
        with self._index_lock:
            ...
            for path in root.rglob("*"):
                ...
                indexed[rel] = {
                    ...
                    "path": rel,
                    "name": path.name,
                    "date": "-".join(rel.split("/")[:3]) if len(rel.split("/")) >= 4 else _mtime_day(path),
