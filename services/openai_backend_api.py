            ...
            if not skip_tasks_on_cf and budget.should_query_tasks():
                try:
                    ...
                    if self._is_cf_edge_block(exc):
                        ...
                        # tasks 仅诊断：记 CF 后仍继续拉 conversation，不空挂
                    else:
                        logger.debug({
    ...
    def _stream_picture_conversation(
        ...
        try:
            ...
            def image_assets_ready(payload: str) -> bool:
                # Prefer leaving SSE as soon as generation is underway or assets exist.
                if FILE_SERVICE_ID_RE.search(payload) or REAL_IMAGE_FILE_ID_RE.search(payload):
                    return True
                low = payload.lower()
                if '"tool_invoked": true' in low or '"tool_invoked":true' in low:
                    return True
                if "skipped_mainline" in low and ("image_gen" in low or "image gen" in low):
                    return True
                return False
