    ...
    def refresh_clearance(
        ...
        try:
            ...
            new_bundle = provider.get_clearance(
                target_url,
                proxy_url=_flaresolverr_proxy_url(profile),
...
def _flaresolverr_proxy_url(profile: ProxyRuntimeProfile) -> str:
    """Return the proxy URL as seen from the FlareSolverr container.

    Local desktop runs talk to FlareSolverr through Windows localhost, while
    FlareSolverr itself runs inside Docker. Passing ``127.0.0.1:40080`` to
    FlareSolverr makes Chromium try to use a proxy inside the FlareSolverr
    container. In the bundled Docker proxy stack the container-visible proxy
    endpoint is ``http://privoxy:8118``.

    The returned URL is only used for the FlareSolverr request payload. Cache
    keys and returned bundles still use the application-visible proxy URL.
    """

    explicit = normalize_proxy_url(str(profile.clearance.get("flaresolverr_proxy_url") or ""))
    if explicit:
        return explicit

    proxy_url = normalize_proxy_url(profile.proxy_url)
    flaresolverr_url = str(profile.clearance.get("flaresolverr_url") or "")
    proxy_host = _host_from_url(proxy_url)
    flaresolverr_host = _host_from_url(flaresolverr_url)
    try:
    ...
    if proxy_host in local_hosts and flaresolverr_host in local_hosts and proxy_port == 40080:
        return "http://privoxy:8118"
    return proxy_url
