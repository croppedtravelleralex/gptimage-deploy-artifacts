...
tempmail_lol_rate_lock = Lock()
tempmail_lol_next_create_at = 0.0
tempmail_lol_backoff_until = 0.0


def _float_setting(value: Any, default: float, minimum: float = 0.0, maximum: float | None = None) -> float:
    try:
...
def _reserve_tempmail_lol_create_slot(min_interval_sec: float) -> None:
    """串行化 TempMail.lol 创建邮箱请求，避免线程池瞬时打爆 free tier。

    注册线程可以继续并发执行 OpenAI/验证码/验号步骤，但 `/inbox/create`
    是 provider 侧共享限额，必须在进程内做全局节流。
    """
    global tempmail_lol_next_create_at
    with tempmail_lol_rate_lock:
        now = time.monotonic()
        wait_until = max(tempmail_lol_next_create_at, tempmail_lol_backoff_until)
        if wait_until > now:
            time.sleep(wait_until - now)
            now = time.monotonic()
        interval = max(0.0, float(min_interval_sec or 0.0))
        tempmail_lol_next_create_at = max(tempmail_lol_next_create_at, now + interval)


def _note_tempmail_lol_rate_limit(backoff_sec: float) -> None:
    global tempmail_lol_backoff_until, tempmail_lol_next_create_at
    backoff = max(0.0, float(backoff_sec or 0.0))
    if backoff <= 0:
        return
    with tempmail_lol_rate_lock:
        until = time.monotonic() + backoff
        tempmail_lol_backoff_until = max(tempmail_lol_backoff_until, until)
        tempmail_lol_next_create_at = max(tempmail_lol_next_create_at, until)


def _retry_after_seconds(value: Any, default: float) -> float:
    text = str(value or "").strip()
    if text:
        try:
            return max(0.0, min(3600.0, float(text)))
        except ValueError:
            pass
...
def _is_transient_mail_error(error: Exception | str | None) -> bool:
    """判断邮箱链路是否只是代理/网络瞬断，避免把可用邮箱池凭据误判为坏件。"""
...
def _is_outlook_token_invalid_error(error: Exception | str | None) -> bool:
    """判断 Outlook refresh_token 是否确实失效，而不是代理链路抖动。"""
    text = str(error or "").lower()
    if not text:
        return False
    invalid_markers = (
        ...
        "provided value for refresh_token is not valid",
        "aadsts",
        "http 400",
        "http 401",
        "http 403",
...
class TempMailLolProvider(BaseMailProvider):
    ...
    def __init__(self, entry: dict, conf: dict):
        ...
        create_interval = entry.get("create_min_interval_sec")
        if create_interval in (None, ""):
            create_interval = entry.get("rate_limit_interval_sec")
        self.create_min_interval_sec = _float_setting(
            create_interval,
            12.0,
            0.0,
            300.0,
        )
        self.rate_limit_backoff_sec = _float_setting(
            entry.get("rate_limit_backoff_sec"),
            60.0,
            1.0,
            3600.0,
        )
...
def mark_mailbox_result(mailbox: dict, *, success: bool, error: Exception | str | None = None) -> None:
    ...
    仅对 outlook_token 邮箱生效：成功标记 used；凭据失效标记 token_invalid；
    代理/WARP/TLS/超时等瞬断释放回未使用；其余业务失败才标记 failed。
    """
    ...
    if _is_transient_mail_error(error):
        _release_outlook_token_state(address)
    elif isinstance(error, OutlookTokenError) and _is_outlook_token_invalid_error(error):
        _set_outlook_token_state(address, "token_invalid", reason[:300])
    elif "OutlookToken 刷新失败" in reason and _is_outlook_token_invalid_error(error):
        _set_outlook_token_state(address, "token_invalid", reason[:300])
    elif "access_token" in reason and _is_outlook_token_invalid_error(error):
        _set_outlook_token_state(address, "token_invalid", reason[:300])
    else:
