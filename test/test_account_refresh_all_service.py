...
class AccountRefreshAllServiceTests(unittest.TestCase):
    ...
    def test_available_account_is_queued_when_panda_auth_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            account_service = AccountService(JSONStorageBackend(Path(tmp_dir) / "accounts.json"))
            account_service.add_account_items([
                {
            ...
            self.assertEqual((synced, failed, queued), (0, 0, 1))
            self.assertTrue(pending_path.exists())
            payload = pending_path.read_text(encoding="utf-8")
            self.assertIn("quota-token", payload)
    ...
    def test_panda_sync_skips_tainted_ready_accounts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual((synced, failed, queued), (0, 0, 1))
            payload = refresh_all._pending_file.read_text(encoding="utf-8")
            self.assertIn("clean-token", payload)
            self.assertNotIn("tainted-token", payload)
