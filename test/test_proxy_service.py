...
class ProxyServiceTests(unittest.TestCase):
    ...
    def test_flaresolverr_local_proxy_is_translated_for_docker_container(self) -> None:
        runtime = make_runtime(
            enabled=True,
            egress_mode="single_proxy",
        ...
        self.assertEqual(provider.proxy_url, "http://privoxy:8118")
        self.assertIsNotNone(bundle)
        assert bundle is not None
        self.assertEqual(bundle.proxy_url, "http://127.0.0.1:40080")

    def test_flaresolverr_proxy_url_can_be_overridden(self) -> None:
        runtime = make_runtime(
            enabled=True,
            egress_mode="single_proxy",
            proxy_url="http://127.0.0.1:40080",
            clearance={
                ...
                "mode": "flaresolverr",
                "flaresolverr_url": "http://127.0.0.1:8191",
                "flaresolverr_proxy_url": "http://proxy-in-container:8118",
            },
        )
        ...
        class FakeProvider:
            ...
            def get_clearance(self, target_url: str, proxy_url: str = "", timeout_sec: int = 60) -> ClearanceBundle | None:
                self.proxy_url = proxy_url
                return ClearanceBundle(target_host="chatgpt.com", proxy_url=proxy_url, user_agent="Flare UA")

        provider = FakeProvider()
        store = ProxySettingsStore(FakeConfig(runtime=runtime), clearance_provider_factory=lambda _url: provider)
        store.refresh_clearance(target_url="https://chatgpt.com", force=True)

        self.assertEqual(provider.proxy_url, "http://proxy-in-container:8118")
