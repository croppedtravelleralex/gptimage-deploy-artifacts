...
class FakeProxyProfile:
    def __init__(self, clearance_enabled=True):
...
class FakeOAuthResponse(FakeResponse):
    def __init__(self, status_code=200, payload=None, text="", headers=None, url="https://auth.openai.com/api/accounts/oauth/token"):
        super().__init__(status_code=status_code, text=text, headers=headers, url=url)
...
class FakeProxySettings:
    def __init__(self, bundle=None, clearance_enabled=True):
        self.bundle = bundle
        self.clearance_enabled = clearance_enabled
    ...
    def get_profile(self, **kwargs):
        return FakeProxyProfile(clearance_enabled=self.clearance_enabled)
...
class RegisterProxyRuntimeTests(unittest.TestCase):
    ...
    def test_oauth_token_exchange_reports_http_detail_without_leaking_tokens(self):
        class TokenSession(FakeSession):
            def post(self, *args, **kwargs):
                return FakeOAuthResponse(
                    status_code=400,
                    payload={"error": "invalid_grant", "access_token": "secret-access-token"},
                    headers={"content-type": "application/json", "x-request-id": "req-test"},
                )

        with self.assertRaisesRegex(RuntimeError, "token换取失败: HTTP 400") as ctx:
            openai_register.request_platform_oauth_token(TokenSession(), "auth-code", "verifier")

        message = str(ctx.exception)
        self.assertIn("invalid_grant", message)
        self.assertIn("x-request-id=req-test", message)
