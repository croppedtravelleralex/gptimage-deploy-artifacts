...
import { runBounded } from "@/lib/bounded-queue";
import { formatPoolQuotaDetail, formatPoolQuotaFromStats, formatQuotaRefreshAgeFromIso } from "@/lib/image-quota";
...
const IMAGE_PROMPT_ENHANCE_STORAGE_KEY = "chatgpt2api:image_prompt_enhance";
const IMAGE_SUBMIT_CONCURRENCY = 4;
...
function taskDataToStoredImage(image: StoredImage, task: ImageTask): StoredImage {
  if (task.status === "success") {
    ...
    return {
      ...
      error: undefined,
      durationMs: task.total_wall_ms ?? task.wall_clock_ms ?? task.phase_timings_ms?.wall_clock_ms ?? task.duration_ms,
    };
  }
...
export default function ImagePageContent({ isAdmin }: { isAdmin: boolean }) {
  ...
  const [promptEnhanceLocale, setPromptEnhanceLocale] = useState("en");
  ...
  const [availableQuotaDetail, setAvailableQuotaDetail] = useState("");
  const [quotaRefreshAge, setQuotaRefreshAge] = useState("");
  ...
  const loadQuota = useCallback(async () => {
    if (!isAdmin) {
      setAvailableQuota("--");
      setAvailableQuotaDetail("");
      setQuotaRefreshAge("");
      return;
    }
    try {
      const data = await fetchAccounts();
      setAvailableQuota(formatPoolQuotaFromStats(data.stats));
      setAvailableQuotaDetail(formatPoolQuotaDetail(data.stats));
      setQuotaRefreshAge(formatQuotaRefreshAgeFromIso(data.stats?.latest_quota_refresh_at));
    } catch {
  ...
  const runConversationQueue = useCallback(
    async (conversationId: string) => {
      ...
      try {
        ...
        const submitResults = await runBounded(
      ...
      <section className="mx-auto grid h-[calc(100dvh-6.5rem)] min-h-0 w-full max-w-[1580px] grid-cols-1 gap-2 overflow-hidden px-0 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] sm:h-[calc(100dvh-5.25rem)] sm:gap-3 sm:px-3 sm:pb-6 lg:grid-cols-[200px_minmax(0,1fr)_200px]">
        ...
        <div className="flex min-h-0 flex-col gap-2 sm:gap-4">
          ...
          <ImageComposer
            ...
            availableQuotaDetail={availableQuotaDetail}
            quotaRefreshAge={quotaRefreshAge}
            ...
            promptEnhanceLocale={promptEnhanceLocale}
            onPromptEnhanceChange={setPromptEnhance}
            onPromptEnhanceLocaleChange={setPromptEnhanceLocale}
