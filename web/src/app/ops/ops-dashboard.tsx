...
import { ImagePipelineGantt } from "@/components/ops/ImagePipelineGantt";
...
import {
  ...
  fetchRiskCalendar,
  fetchRiskChecks,
  fetchWebshareCfScanStatus,
  runOpsAgent,
  runRiskCheck,
  runWebshareCfScanOnce,
  setNurtureEnabled,
} from "@/lib/api";
import { humanizeRiskFinding } from "@/lib/chat-format";

type TabId = "rca" | "nurture" | "risk" | "proxy";
...
export default function OpsDashboard() {
  ...
  const [proxyScan, setProxyScan] = useState<Record<string, unknown> | null>(null);
  ...
  const loadProxyScan = async () => {
    try {
      const data = await fetchWebshareCfScanStatus();
      setProxyScan(data as unknown as Record<string, unknown>);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "加载 Webshare CF 扫描状态失败");
    }
  };
  ...
  const scanProxyBatch = async () => {
    setBusy(true);
    try {
      await runWebshareCfScanOnce();
      toast.success("已触发一批 Webshare CF 扫描");
      await loadProxyScan();
    } catch (error) {
  ...
  const proxyCounts = (proxyScan?.counts || {}) as Record<string, number>;
  const proxyInventory = (proxyScan?.inventory || {}) as Record<string, unknown>;
  const cf403Nodes = Array.isArray(proxyInventory.cf403_nodes) ? proxyInventory.cf403_nodes : [];
  const availableNodes = Array.isArray(proxyInventory.available_endpoints) ? proxyInventory.available_endpoints : [];
      ...
      {tab === "proxy" ? (
        <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
          <CardContent className="space-y-4 p-5">
            <div className="flex items-center justify-between gap-2">
              <div className="text-sm font-medium text-stone-800">Webshare 代理池 CF403</div>
              <Button variant="ghost" className="h-8 rounded-lg px-2" onClick={() => void loadProxyScan()}>
                <RefreshCw className="size-4" />
              </Button>
            </div>
            <p className="text-xs leading-5 text-stone-500">
              不定期扫描 Webshare 池（home + requirements/prepare）。CF403 节点自动写入隔离名单，换绑时不再分配。
            </p>
            <div className="grid grid-cols-2 gap-2 text-sm text-stone-600 md:grid-cols-4">
              <div>
                池总量: <span className="font-medium text-stone-900">{String(proxyCounts.pool_total ?? 0)}</span>
              </div>
              <div>
                可用: <span className="font-medium text-emerald-700">{String(proxyCounts.available_count ?? 0)}</span>
              </div>
              <div>
                CF403: <span className="font-medium text-rose-700">{String(proxyCounts.cf403_count ?? 0)}</span>
              </div>
              <div>
                扫描: <Badge className="rounded-md">{boolZh(proxyScan?.enabled)}</Badge>
              </div>
            </div>
            <div className="grid grid-cols-1 gap-2 text-xs text-stone-500 md:grid-cols-2">
              <div>最近扫描: {String(proxyScan?.last_scan_at || "—")}</div>
              <div>下次扫描: {String(proxyScan?.next_scan_at || "—")}</div>
            </div>
            {proxyScan?.last_error ? <p className="text-xs text-rose-600">{String(proxyScan.last_error)}</p> : null}
            <Button
              variant="outline"
              className="rounded-xl"
              disabled={busy}
              onClick={() => void scanProxyBatch()}
            >
              立即扫描一批
            </Button>

            <div className="space-y-2">
              <div className="text-sm font-medium text-rose-800">CF403 节点（{cf403Nodes.length}）</div>
              <div className="max-h-56 overflow-auto rounded-xl border border-rose-100 bg-rose-50/40">
                {cf403Nodes.length === 0 ? (
                  <p className="p-3 text-xs text-stone-500">暂无 CF403 节点</p>
                ) : (
                  <table className="w-full text-left text-xs text-stone-700">
                    <thead className="sticky top-0 bg-rose-50 text-stone-500">
                      <tr>
                    ...
                    <tbody>
                      {cf403Nodes.map((node) => {
                        ...
                        return (
                          <tr key={String(row.endpoint)} className="border-t border-rose-100/80">
                            <td className="px-3 py-2 font-mono">{String(row.endpoint || "—")}</td>
                            <td className="px-3 py-2">{String(row.reason || row.cf_classification || "cf403")}</td>
                            <td className="px-3 py-2">{String(row.egress_ip || "—")}</td>
                            <td className="px-3 py-2">
                              home={String(row.home_status ?? "—")} / req={String(row.requirements_status ?? "—")}
                            </td>
                          </tr>
                        );
                      })}
            ...
            <div className="space-y-2">
              <div className="text-sm font-medium text-emerald-800">可用节点（{availableNodes.length}）</div>
              <div className="max-h-56 overflow-auto rounded-xl border border-emerald-100 bg-emerald-50/30">
                {availableNodes.length === 0 ? (
                  <p className="p-3 text-xs text-stone-500">暂无可用节点</p>
                ) : (
                  <table className="w-full text-left text-xs text-stone-700">
                    <thead className="sticky top-0 bg-emerald-50 text-stone-500">
                      <tr>
                        <th className="px-3 py-2">节点</th>
                        <th className="px-3 py-2">出口 IP</th>
                        <th className="px-3 py-2">最近探测</th>
                      </tr>
                    </thead>
                    <tbody>
                      {availableNodes.map((node) => {
                        const row = node as Record<string, unknown>;
                        return (
                          <tr key={String(row.endpoint)} className="border-t border-emerald-100/80">
                            <td className="px-3 py-2 font-mono">{String(row.endpoint || "—")}</td>
                            <td className="px-3 py-2">{String(row.egress_ip || "—")}</td>
                            <td className="px-3 py-2">
                              {row.last_probe_ok === true
                                ? "OK"
                                : row.last_probe_ok === false
                                  ? "失败"
                                  : "未扫描"}
                            </td>
                          </tr>
