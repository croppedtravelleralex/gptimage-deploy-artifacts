...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  return {
    ...
    image_account_concurrency: Number(config.image_account_concurrency || 2),
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  saveConfig: async () => {
    ...
    try {
      const data = await updateSettingsConfig({
        ...
        image_account_concurrency: Math.max(1, Number(config.image_account_concurrency) || 2),
        image_settle_enabled: Boolean(config.image_settle_enabled !== false),
        image_check_before_hit_enabled: Boolean(config.image_check_before_hit_enabled !== false),
  ...
  toggleRegister: async () => {
    ...
    try {
      ...
      } else if (data.register.enabled) {
        toast.success("注册任务已启动");
      } else {
        const lastLog = [...(data.register.logs ?? [])].reverse().find((item) => item.level === "red");
        toast.error(lastLog?.text ?? "注册任务未启动，请检查实时日志");
      }
    } catch (error) {
